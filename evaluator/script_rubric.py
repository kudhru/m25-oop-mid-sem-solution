...
class RubricEvaluator:
    def __init__(self, student_file: str, solution_dir: str):
        ...
        self.rubrics = {
            ...
            2: {
                ...
                "compilation_marks": 2,
                "subtasks": {
                    "2.1": {"marks": 1, "description": "Same building, different room number"},
                    "2.2": {"marks": 1, "description": "Different building, same room number"},
                    "2.3": {"marks": 1, "description": "Different building and room number"},
            ...
            3: {
                ...
                "subtasks": {
                    ...
                    "3.2": {"marks": 2, "description": "Same building, different room numbers"}
                }
            },
            4: {
                "name": "addRoom",
                ...
                "subtasks": {
                    ...
                    "4.5": {"marks": 1.5, "description": "Initialize bookingsByRoomKey"},
            ...
            9: {
                ...
                "subtasks": {
                    ...
                    "9.4": {"marks": 2, "description": "Return ALREADY_BOOKED if booked"}
                }
            },
            10: {
                ...
                "compilation_marks": 2,
                "subtasks": {
                    "10.1": {"marks": 1, "description": "Invalid hour returns empty list"},
    ...
    def extract_function(self, content: str, task_num: int) -> Tuple[str, bool]:
        ...
        if task_num == 3:
            pattern = r'public\s+static\s+final\s+java\.util\.Comparator<Room>\s+BY_BUILDING_THEN_ROOM\s*=\s*(.*?);'
        elif task_num == 7:
            ...
            if task_num == 1:
                ...
                pattern = r'public\s+ErrorCode\s+bookRoom\s*\([^)]+\)\s*\{(.*?)\n\s{2}\}'
            elif task_num == 9:
        ...
        if not extracted or extracted == "/* Write your code here */" or len(extracted) < 5:
            return "", True
    ...
    def create_test_file(self, task_num: int, student_impl: str) -> str:
        ...
        modified_content = re.sub(pattern, replacement, solution_content, flags=re.DOTALL)
    ...
    def parse_test_results(self, output: str, task_num: int) -> Dict[str, Dict[str, Any]]:
        ...
        for subtask_id in rubric["subtasks"].keys():
            ...
            if match:
                ...
                results[subtask_id] = {
                    "passed": passed,
                    "marks_awarded": rubric["subtasks"][subtask_id]["marks"] if passed else 0,
    ...
    def evaluate_task(self, task_num: int, student_impl: str) -> Dict[str, Any]:
        ...
        for subtask_id in self.rubrics[task_num]["subtasks"]:
            subtasks[subtask_id] = {
                "passed": False,
                "marks_awarded": 0,
                "maximum_marks": self.rubrics[task_num]["subtasks"][subtask_id]["marks"]
            }
        ...
        result = {
            ...
            "total_maximum_marks": sum(self.rubrics[task_num]["subtasks"][st]["marks"] for st in self.rubrics[task_num]["subtasks"]) + self.rubrics[task_num]["compilation_marks"]
        }
        ...
        # Calculate marks
        subtask_marks = sum(subtask_results[st]["marks_awarded"] for st in subtask_results)
        
        # Award compilation marks only if all subtasks passed
        all_passed = all(subtask_results[st]["passed"] for st in subtask_results)
        if all_passed:
            result["compilation_marks_awarded"] = self.rubrics[task_num]["compilation_marks"]
        
        result["total_marks_awarded"] = subtask_marks + result["compilation_marks_awarded"]
    ...
    def evaluate(self) -> Dict[str, Any]:
        ...
        for task_num in range(1, 11):
            ...
            if not_impl:
                ...
                results[f"task_{task_num}"] = {
                    ...
                    "total_marks_awarded": 0,
                    "total_maximum_marks": sum(rubric["subtasks"][st]["marks"] for st in rubric["subtasks"]) + rubric["compilation_marks"]
                }
            else:
        ...
        report = {
            ...
            "percentage": round((total_marks / max_total_marks) * 100, 2) if max_total_marks > 0 else 0,
            "tasks": results
        }
...
class BatchRubricEvaluator:
    ...
    def find_student_java_file(self, student_dir: str) -> str:
    ...
    def process_student(self, student_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            import shutil
            student_output_file = os.path.join(output_dir, os.path.basename(student_file))
            ...
            print(f"  Completed: {report['total_marks']}/{report['max_marks']} marks ({report['percentage']}%)")
            return report
    ...
    def create_summary_report(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for student_id, report in results.items():
            student_summary = {
                ...
                "id_number": report.get("student_info", {}).get("id_number", student_id),
        ...
        summary["overall_stats"] = {
            ...
            "average_percentage": round((total_marks_all / max_marks_all * 100) if max_marks_all > 0 else 0, 2)
        }
...
def main():
    ...
    batch_evaluator = BatchRubricEvaluator(student_solutions_dir, solution_dir, output_dir)
    try:
        ...
        if results:
            ...
            for student_id, report in results.items():
                print(f"{student_id}: {report['total_marks']}/{report['max_marks']} marks ({report['percentage']}%)")
